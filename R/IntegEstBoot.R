...
#'
#' This function implements the nonparametric bootstrap inference on the
#' dose-response curve via our integral estimator.
#'
...
#' \item{m_est}{The estimated dose-response curve evaluated at points \code{t_eval}.}
#' \item{m_est_boot}{The estimated dose-response curve evaluated at points \code{t_eval} for all the bootstrap samples.}
#' \item{m_alpha}{The width of the uniform confidence band.}
#' \item{m_alpha_var}{The widths of the pointwise confidence bands at evaluation points \code{t_eval}.}
#'
...
#'
#' @keywords estimator integral our through curve dose-response the on inference bootstrap
#'
#' @examples
#' \donttest{
#'   set.seed(123)
...
IntegEstBoot <- function(Y, X, t_eval = NULL, boot_num = 500, alpha=0.95,
                         h_bar = NULL, kernT_bar = "gaussian", h = NULL, b = NULL,
                         C_h = 7, C_b = 3, print_bw = TRUE, degree = 2,
                         deriv_ord = 1, kernT = "epanechnikov",
  ...
  m_boot_sup <- apply(abs(sweep(m_est_boot, 2, m_est)), 1, max)
  m_alpha <- quantile(m_boot_sup, alpha, na.rm = TRUE)

  # Compute the alpha% pointwise confidence intervals
  m_boot_abs <- abs(sweep(m_est_boot, 2, m_est))
  m_alpha_var <- apply(m_boot_abs, 2, quantile, probs = alpha, na.rm = TRUE)

  # Return the results
  return(list(m_est = m_est, m_est_boot = m_est_boot, m_alpha = m_alpha,
              m_alpha_var = m_alpha_var))
}
